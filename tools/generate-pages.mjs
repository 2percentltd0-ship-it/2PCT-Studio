...
const page=(title,description,canonical)=>template.replaceAll('%%TITLE%%',escape(title)).replaceAll('%%DESCRIPTION%%',escape(description)).replaceAll('%%CANONICAL%%',canonical);
const workDir=path.join(root,'work'); fs.mkdirSync(workDir,{recursive:true});
...
for(const [key,category] of Object.entries(categories)){
  ...
  fs.writeFileSync(path.join(categoryDir,'index.html'),page(`${category.title} — 2 Percent`,category.lead,`https://www.2pctstudio.com/work/${key}/`));
  ...
  category.images.forEach((project,index)=>{
    ...
    fs.writeFileSync(path.join(projectDir,'index.html'),page(`${title} — ${category.title} — 2 Percent`,description,`https://www.2pctstudio.com/work/${key}/${slug}/`));
