...
for(const [key,category] of Object.entries(categories)){
  const projectAssets=new Set(category.images.flatMap(project=>[
    project.coverSrc,
    project.src,
    ...(project.gallery||[]).map(asset=>asset.src)
  ]).filter(Boolean));
  if(projectAssets.has(category.cover?.src)){
    throw new Error(`${key}: the homepage category cover must use an independent file, not a project asset`);
  }
